...
int _smallerIndexHeap(DynArr *heap, int i, int j, comparator compare)
{
    ...
    compareResult = compare(heap->data[i],heap->data[j]);
    if(compareResult == -1)
        return i;
    else
        return j;
...
void addHeap(DynArr *heap, TYPE val, comparator  compare)
{
    ...
    addDynArr(heap, val);
    int cur = sizeDynArr(heap)-1;
    int parent = (cur-1)/2;
    while (cur > 0 && compare(heap->data[cur],heap->data[parent]) == -1) {
...
void _adjustHeap(DynArr *heap, int max, int pos, comparator compare)
{
    ...
    if(rightchild < max){
        int smallestchild = _smallerIndexHeap(heap, leftchild , rightchild, compare);
        if(compare(heap->data[pos],heap->data[smallestchild]) == 1){
            swapDynArr(heap, pos, smallestchild);
            _adjustHeap(heap, max, smallestchild, compare);
        }
    }
    else if(leftchild < max){
        if(compare(heap->data[pos],heap->data[leftchild]) == 1){
