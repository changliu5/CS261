...
void _printList(struct linkedList* lst)
{
    int i;
    assert(lst != NULL);
    struct linkedList *temp = lst;    printf("Print from first element: ");
    for(i=0;i<lst->size;i++){
        printf("%d ", temp->firstLink->next->value);
        temp->firstLink = temp->firstLink->next;
    }
    printf("\nPrint from last element: ");
    temp = lst;
    for(i=0;i<lst->size;i++){
        printf("%d ", temp->lastLink->prev->value);
        temp->lastLink = temp->lastLink->prev;
    }
    printf("\n");
    /* FIXME: you must write this */
